...
# read in segmentation_exp and patches
...
experimental_path = model_directory+date_directory+exp_directory+ f'{date}_{time}_{spatial_dim}D_{model_name}_experiment.yml'
...
training_type = "sequential_image_processing"
...
training_raw_list = raw_filename_list[train_idx]
training_mask_list = mask_filename_list[train_idx]
validation_raw_list = raw_filename_list[val_idx]
validation_mask_list = mask_filename_list[val_idx]
...
whole_training_dataset = WholeVolumeDataset(raw_directory = training_raw_list,
...
whole_validation_dataset = WholeVolumeDataset(raw_directory = validation_raw_list,
...
subpatch_training_dataloader = DataLoader(subpatch_training_data, batch_size=1, shuffle=True)
whole_training_dataloader = DataLoader(whole_training_dataset, batch_size=1, shuffle=False)
whole_validation_dataloader = DataLoader(whole_validation_dataset, batch_size=1, shuffle=False)
...
# pth_file = model_directory+date_directory+exp_directory + "+s_+d_-f_ResUNet_0_2.pth"
...
# subpatch_training_dataloader, whole_training_dataloader, whole_validation_dataloader = setup_dataflow(config,
#                                                                                                       raw_filename_list, 
#                                                                                                       mask_filename_list, 
#                                                                                                       train_idx, 
#                                                                                                       val_idx, 
...
# set up optimizer hyperparameters
...
if loss_type == "dice_focal":
    ...
    loss_function = config_loss[loss_type](softmax=True)

# loss_function = FocalLoss()
optimizer = torch.optim.AdamW(model.parameters(), lr = learning_rate, weight_decay = l2)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max = max_epochs/config['end_cycle'], verbose = True)
    ...
    if lower_img == []:
        ...
        if augment:
            ...
            for bindex in range(0, len(upper_indexes), batch_size):
                ...
                sub_imgs, sub_masks = augmentation(all_transform, training_type, spatial_dim, upper_img, upper_mask, upper_batch)
    ...
    # train on both 
        ...
        if augment:
            ...
            for bindex in range(0, len(lower_indexes), batch_size):
                ...
                sub_imgs, sub_masks = augmentation(all_transform, training_type, spatial_dim, lower_img, lower_mask, lower_batch)
                sub_imgs, sub_masks = sub_imgs.to(device), sub_masks.to(device)
                optimizer.zero_grad()
                output = model(sub_imgs)
                probabilities = torch.softmax(output, 1)
...
def save_model(trainer):
    ...
    def log_training_results(trainer):
        evaluator.run(whole_training_dataloader)
    ...
    def log_testing_results(trainer):
        evaluator.run(whole_validation_dataloader)
...
trainer.run(subpatch_training_dataloader, max_epochs = max_epochs)
